...
pub mod pallet {
	...
	use sp_runtime::{RuntimeDebug, offchain as rt_offchain, offchain::{
	...
	struct PolkadotPrice {
		// Specify our own deserializing function to convert JSON string to vector of bytes
		#[serde(deserialize_with = "de_string_to_bytes")]
		#[serde(rename(deserialize = "priceUsd"))]
		pub price_usd: Vec<u8>,
	}

	#[derive(Deserialize, Encode, Decode, Default)]
	struct PolkadotPriceResp {
	...
	impl<T: Config> Pallet<T> {
		...
		fn fetch_price_info() -> Result<(), Error<T>> {
			...
			let url: &str = "https://api.coincap.io/v2/assets/polkadot";
			let raw_price = match Self::fetch_n_parse::<PolkadotPriceResp>(url) {
				Ok(resp) => resp.data.price_usd,
				Err(err) => { return Err(err); }
			};
			let call = Call::submit_price_unsigned(Self::parse_raw_price(raw_price));
		...
		fn parse_raw_price(raw_price: Vec<u8>) -> (u64, Permill) {
			let mut price_integer: u64 = 0;
			let mut price_permill_p: u64 = 0;
			let mut price_permill_q: u64 = 1;
			let mut before_dot = true;
			for byte in raw_price {
				match byte {
					46 => before_dot = false,
					_ => {
						assert!(byte >= 48 && byte <= 57, "Invalid raw price data.");
						if before_dot {
							price_integer = price_integer * 10 + (byte - 48) as u64;
						} else {
							price_permill_p = price_permill_p * 10 + (byte - 48) as u64;
							price_permill_q = price_permill_q * 10;
						}
					}
				}
			}
			(price_integer, Permill::from_rational(price_permill_p, price_permill_q))
		}
		...
		fn fetch_n_parse<E>(url: &str) -> Result<E, Error<T>> where E: DeserializeOwned {
